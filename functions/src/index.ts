...
// ------------------------------------------------------------------
// SEND STAFF INVITATION EMAIL (Firestore Trigger)
// Triggered when a new staffInvitation document is created
// ------------------------------------------------------------------
export const sendStaffInvitationEmail = onDocumentWritten(
  "staffInvitations/{invitationId}",
  async (event) => {
    // Only process on create (not update/delete)
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://eduflow360.app";
    ...
    logger.info("Sending staff invitation email", {
      email,
      assignedRole,
      providerName,
      invitedByName,
    ...
    // Send email using Firebase Extensions (Trigger Email) or custom SMTP
    // This requires the "Trigger Email" extension to be installed
    // Or you can use a third-party service like SendGrid, Mailgun, etc.
    
    try {
      const db = admin.firestore();
      
      // If using Firebase Trigger Email extension, write to "mail" collection
      await db.collection("mail").add({
        to: email,
        message: {
          subject: `You've been invited to join ${providerName || "EduFlow360"}`,
          html: `
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
              ...
              <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
                ...
                <p style="font-size: 16px; margin-bottom: 20px;">
                  <strong>${invitedByName || "Someone"}</strong> has invited you to join 
                  <strong>${providerName || "EduFlow360"}</strong> as a <strong>${roleLabel}</strong>.
                </p>
          ...
          `,
          text: `
You've been invited to join ${providerName || "EduFlow360"}!

${invitedByName || "Someone"} has invited you to join as a ${roleLabel}.
...
If you didn't expect this email, you can safely ignore it.
      ...
      logger.info("Staff invitation email queued successfully", { email });
    } catch (error) {
      logger.error("Failed to send staff invitation email", { error, email });
    }
  }
);
